...
![Image 1](_media/20210421163450.png)
...
- `dns-prefetch`。当 `link` 标签的 `rel` 属性值为“dns-prefetch”时，`浏览器会对某个域名预先进行 DNS 解析并缓存`。这样，当浏览器在请求同域名资源的时候，能省去从域名查询 IP 的过程，从而`减少时间损耗`。下图是淘宝网设置的 DNS 预解析
  ...
  ![Image 1](_media/20210421163848.png)
...
![Image 1](_media/20210421164758.png)
...
> 了解更多关于重绘和重排的样式属性，可以参看这个网址：[https://csstriggers.com/(opens new window)][https_csstriggers.com_opens new window]。
