...
- 函数声明创建函数时，函数的 `prototype` 属性被自动设置为一个继承自 `Object.prototype` 的对象，该对象有个自有属性 `constructor`，其值就是函数本身。
  ...
  console.log(YourConstructor.prototype.**proto** === Object.prototype) // true
...
![Image 1](_media/50038a46298f4da8a7b4a10bbc812f47.png)
...
![Image 1](_media/0cf0c6367fae4b2fafaf3e7bf63dc9e3.png)
...
![Image 1](_media/bbd0e8a847c24b1c95fbf410f9e0eeb7.png)
...
![Image 1](_media/424852a811c145448801e334af5c1408.png)
...
![Image 1](_media/a1cfb4b5ea114d258b6ae69f0b71009d.png)
...
![Image 1](_media/12bf7e191e0c4d2b936d36e2f8159abc.png)
...
![Image 1](_media/3d1558af8a0641fb8c3b1a6e1f0151c0.png)
...
![Image 1](_media/1f09786533fa48bea807a094532862da.png)
...
![Image 1](_media/0924d3d6d060459590283ef9c70b4f2a.png)
...
![Image 1](_media/11cee8c2035b4c7197747bc2a7a1c886.png)
...
![Image 1](_media/1259b8f5ded745beabaac6dd066f3e76.png)
...
![Image 1](_media/2f05aab6a7da4922841b6a9bca7a312e.png)
...
![Image 1](_media/411563a867934d779d0b09489230c339.png)
...
![Image 1](_media/c72aa4f1a84a40a2b376df74681de2f2.png)
...
![Image 1](_media/baabff95e91b4fb59c8b7c847fc820e0.png)
...
![Image 1](_media/5bbb0f776f884ab6baeeb913650add0f.png)
...
![Image 1](_media/01df07ca28f9454e80449baf48e603d6.png)
...
![Image 1](_media/8e331230e47343fab4869bcf53f94c8e.png)
...
![Image 1](_media/9ec5ec0f95424de0a838d6491c54672e.png)
...
![Image 1](_media/c12f6c9d21624ef083685ed003944a37.png)
...
- 我们可以改造一下，将计算折扣的算法部分提取出来保存为一个对象，折扣的类型作为 `key`，这样索引的时候通过对象的键值索引调用具体的算法：
  ...
  const DiscountMap = {
  minus100*30: function(price) {
  return price - Math.floor(price / 100) * 30
  },
  minus200*80: function(price) {
  return price - Math.floor(price / 200) * 80
  },
...
![Image 1](_media/ec1e2d4640574580952503e4bb1ff1eb.png)
...
![Image 1](_media/dc11afe47883428596ac2dfb735ba96d.png)
...
- 我们可以像官网示例一样把表单验证都写在组件的状态 data 函数中，但是这样就不好复用使用频率比较高的表单验证方法了，这时我们可以结合策略模式和函数柯里化的知识来重构一下。首先我们在项目的工具模块（一般是 utils 文件夹）实现通用的表单验证方法：
  ...
  /_ 姓名校验 由 2-10 位汉字组成 _/
  export function validateUsername(str) {
  const reg = /^[\u4e00-\u9fa5]{2,10}$/
  return reg.test(str)
  }

  /_ 手机号校验 由以 1 开头的 11 位数字组成 _/
...
![Image 1](_media/a704d99ff9d4449e808fc373ba75da4c.png)
...
![Image 1](_media/1dc5241a710e40b69f065cefbd6cbfa3.png)
...
![Image 1](_media/7605657ecb0a4da5bf3f4e51088c112b.png)
...
\`\`js // jquery 源码 /src/core.jsL246-L265 each: function (obj, callback) \{ var i = 0
