...
关于 ES6/7 的考查内容还有很多，本小节就不逐一介绍了，如果想继续深入学习，可以在线看《[ES6 入门(opens new window)][es6_opens new window]》。
...
![Image 1](_media/41785f7b0ea043429d675b939687697d.png)
...
> 目前已经有一个获取 HTTP 请求更加方便的 API：`Fetch`，通过`Fetch`提供的`fetch()`这个全局函数方法可以很简单地发起异步请求，并且支持`Promise`的回调。但是 Fetch API 是比较新的 API，具体使用的时候还需要查查 [caniuse(opens new window)][caniuse_opens new window]，看下其浏览器兼容情况。
...
![Image 1](_media/00c8ffe616ec4e8880bcdef8d6f0dd5a.png)
...
![Image 1](_media/26d5ab81fe32448188e78fd95d9ebfe6.png)
...
![Image 1](_media/3f0766d9d1684722bdb55d3ddb2051f2.png)
...
![Image 1](_media/1f28e1164e584fe1903e0a7f460fd5f3.png)
...
![Image 1](_media/a1c34174837d4506b59ce4c752161443.png)
...
![Image 1](_media/0fc3b8ba9d3e410eb3c42f6b865ac8fa.png)
...
![Image 1](_media/fb3895433db44d99b300431bf5686e6b.png)
...
![Image 1](_media/a9b80b727e914cb18d3b456c75b17bea.png)

> 加上`float:left`之后：

![Image 1](_media/d72a2e0671244ccd8d0e6f29311859a4.png)
...
> float 的设计初衷是解决文字环绕图片的问题，后来误打误撞用于做布局，因此有许多不合适或者需要注意的地方，上文基本都讲到了需要的知识点。如果是刚开始接触 float 的同学，学完上面的基础知识之后，还应该做一些练习实战一下 —— 经典的“圣杯布局”和“双飞翼布局”。这里就不再展开讲了，网上资料非常多，例如[浅谈面试中常考的两种经典布局——圣杯与双飞翼(opens new window)][opens new window]（此文的最后两张图清晰地展示了这两种布局）。
...
![Image 1](_media/5773ddf33e0141e2a763bbc09c31eef2.png)
...
![Image 1](_media/3dbb36f3d0a34843856f463bca14eb73.png)
...
![Image 1](_media/09f594d6d5ec4011bfcaa598bdfcaa47.png)
...
![Image 1](_media/78f40f3591124b67b33dbe1b431a246b.png)

> 然后，我们把第三个`<p>`改为`position:absolute;`，看看会发生什么变化。

![Image 1](_media/1cc71fe0e3ce4e62bc6fc99b98b6175c.png)
...
![Image 1](_media/2529970d320a4a7ea211a7ae4fafccb7.png)

> `fixed` 元素的定位是相对于 `window` （或者 `iframe`）边界的，和其他元素没有关系。但是它具有破坏性，会导致其他元素位置的变化。

![Image 1](_media/bb6773dce3234229998862238ef5837e.png)
...
![Image 1](_media/75f21226c8324eec9188961d8a910f26.png)

![Image 1](_media/c63754d4b6ea45c1b289a512d59418a8.png)
...
任何一个容器都可以使用 flex 布局，代码也很简单。
    ...
    <style type="text/css"> .container { display: flex; } .item { border: 1px solid #000; flex: 1; } </style>
...
![Image 1](_media/9a82a6ce9011462f962a3cd57ac0a7a4.png)
...
![Image 1](_media/87ba2560667b42d1821ef157365e2bc6.png)
...
![Image 1](_media/4cd2b5ffb09c4a38b61c9fe79c1fed2e.png)
...
![Image 1](_media/4f7a547fd44f42c3b9edc7efaf9e07a4.png)
...
![Image 1](_media/e75fb6f91e504188bc698a2692a7d635.png)
...
> 对于零宽断言的详细介绍可以阅读「[零宽断言(opens new window)][opens new window 1]」这篇文章。
...
![Image 1](_media/86edb71ee7ef47999a9095ee10609cbd.png)
...
> 关于浏览器整个流程，百度的多益大神有更加详细的文章，推荐阅读下：《[从输入 URL 到页面加载完成的过程中都发生了什么事情？(opens new window)][url _opens new window] 》。
...
- 前端最常用的 IDE 有 [Webstorm(opens new window)][webstorm_opens new window]、[Sublime(opens new window)][sublime_opens new window]、[Atom(opens new window)][atom_opens new window] 和 [VSCode(opens new window)][vscode_opens new window]，我们可以分别去它们的官网看一下。
...
- 你此前做过的项目一定要用过 Git，而且必须是命令行，如果没用过，你自己也得恶补一下。对 Git 的基本应用比较熟悉的同学，可以跳过这一部分了。macOS 自带 Git，Windows 需要安装 Git 客户端，去 [Git 官网(opens new window)][git _opens new window] 下载即可。
...
- 最早普及使用的构建工具是 [Grunt(opens new window)][grunt_opens new window] ，不久又被 [Gulp(opens new window)][gulp_opens new window] 给追赶上。Gulp 因其简单的配置以及高效的性能而被大家所接受，也是笔者个人比较推荐的构建工具之一。如果你做一些简单的 JS 开发，可以考虑使用。
...
- 首先，你需要安装 Node.js，没有安装的可以去 [Node.js 官网(opens new window)][node.js _opens new window] 下载并安装。安装完成后运行如下命令来验证是否安装成功。
...
- 针对移动端的抓包工具，Mac 系统下推荐使用 Charles 这个工具，首先 [下载(opens new window)][opens new window 2] 并安装，打开。Windows 系统推荐使用 [Fiddler(opens new window)][fiddler_opens new window]，下载安装打开。两者使用基本一致，下面以 Charles 为例介绍。
...
![Image 1](_media/a26b57bf2d554e708c091dcda43f38df.png)
...
> [如何在跳槽前客观地评估自己的身价？(opens new window)][opens new window 3]
...
[es6_opens new window]: http://es6.ruanyifeng.com/
[caniuse_opens new window]: https://caniuse.com/
[opens new window]: https://juejin.im/entry/5a8868cdf265da4e7e10c133?utm_source=gold_browser_extension
[opens new window 1]: https://deerchao.net/tutorials/regex/regex.htm#lookaround
[url _opens new window]: http://fex.baidu.com/blog/2014/05/what-happen/
[webstorm_opens new window]: https://www.jetbrains.com/webstorm/
[sublime_opens new window]: https://www.sublimetext.com/
[atom_opens new window]: https://atom.io/
[vscode_opens new window]: https://code.visualstudio.com/
[git _opens new window]: https://git-scm.com/download/win
[grunt_opens new window]: http://www.gruntjs.net/
[gulp_opens new window]: https://www.gulpjs.com.cn/
[node.js _opens new window]: http://nodejs.cn/download/
[opens new window 2]: https://www.charlesproxy.com/download/
[fiddler_opens new window]: http://rj.baidu.com/soft/detail/10963.html
[opens new window 3]: https://www.zhihu.com/question/28313219/answer/331599918
