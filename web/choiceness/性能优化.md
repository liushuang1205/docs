...
![Image 1](_media/cdc4135ab1144a8cbfb291434b399928.png)
...
![Image 1](_media/9c3057e1263c40a8a48ff190f7ab3a34.png)
...
相信每个用过 webpack 的同学都对“打包”和“压缩”这样的事情烂熟于心。这些老生常谈的特性，我更推荐大家去阅读文档。而关于 webpack 的详细操作，则推荐大家读读这本 [关于 webpack 的掘金小册(opens new window)][webpack _opens new window]，这里我们把注意力放在 webpack 的性能优化上。
...
- `HappyPack` 的使用方法也非常简单，只需要我们把对 `loader` 的配置转移到 `HappyPack` 中去就好，我们可以手动告诉 `HappyPack` 我们需要多少个并发的进程：
  ...
  ...
  new HappyPack({
  // 这个 HappyPack 的“名字”就叫做 happyBabel，和楼上的查询参数遥相呼应
  id: 'happyBabel',
  // 指定进程池
...
这里为大家介绍一个非常好用的包组成可视化工具——[webpack-bundle-analyzer(opens new window)][webpack-bundle-analyzer_opens new window]，配置方法和普通的 plugin 无异，它会以矩形树图的形式将包内各个模块的大小和依赖关系呈现出来，格局如官方所提供这张图所示：

![Image 1](_media/930fef8e79234f32884af5d396e4919d.png)
...
- 举个 🌰，假设我的主干文件（入口文件）是这么写的：
  ...
  // show 是事先定义好的函数，大家理解它的功能是展示页面即可
  show(page1)
...
《高性能网站建设指南》的作者 Steve Souders 曾在 2013 年的一篇 [博客(opens new window)][opens new window] 中提到：
...
> 这里先给大家介绍 [HTTP-Archive(opens new window)][http-archive_opens new window] 这个网站，它会定期抓取 Web 上的站点，并记录资源的加载情况、Web API 的使用情况等页面的详细信息，并会对这些数据进行处理和分析以确定趋势。通过它我们可以实时地看到世界范围内的 Web 资源的统计结果。

截止到 2018 年 8 月，过去一年**总的 web 资源**的平均请求体积是这样的：

![Image 1](_media/a0254c2ba10449128f23d32b0e133f08.png)

而具体到**图片**这一类的资源，平均请求体积是这样的：

![Image 1](_media/96b7e617dc9143b9b73a41a9d4457970.png)
...
![Image 1](_media/af00b0157f144dbfa43be21a4dc5dc33.png)

京东首页也不例外：

![Image 1](_media/fecfed7e6a2547dd84be0407bfa3480c.png)
...
![Image 1](_media/1a0259c7bf284ff99f76195b787fc857.png)

较小的 Logo：

![Image 1](_media/9dd152acc3ad49ccbe130ef95973340e.png)

颜色简单、对比度较强的透明小图也在 PNG 格式下有着良好的表现：

![Image 1](_media/c46539944e004efba879e4381c35fa67.png)
...
> 在实际开发中，我们更多用到的是后者。很多情况下设计师会给到我们 SVG 文件，就算没有设计师，我们还有非常好用的 [在线矢量图形库(opens new window)][opens new window 1]。对于矢量图，我们无须深究过多，只需要对其核心特性有所掌握、日后在应用时做到有迹可循即可。
...
![Image 1](_media/6699ed02c0fb4b76ae362a94cb20cfa1.png)
...
![Image 1](_media/9d44460085f7434fb1946aeafac8159b.png)
...
这里最推荐的是利用 webpack 来进行 Base64 的编码——webpack 的 [url-loader(opens new window)][url-loader_opens new window] 非常聪明，它除了具备基本的 Base64 转码能力，还可以结合文件大小，帮我们判断图片是否有必要进行 Base64 编码。
...
![Image 1](_media/a5bc16295df54b629d2e577301082ba1.png)
...
![Image 1](_media/547092a64da14c2d958efd8f1e91baad.png)
...
![Image 1](_media/d406bf78cd5546ca84ca855ea7314864.png)
...
![Image 1](_media/533b31561b5f40019018d1c67d48f98f.png)

我们给 `size` 这一栏一个特写：

![Image 1](_media/ecd2078cb64b4a7eb3c173203bc35726.png)
...
![Image 1](_media/5d26377dd66f42b684aea65091c7c5a9.png)
...
![Image 1](_media/529b900fb7e1482e8e25306a64f582bc.png)
...
![Image 1](_media/7cec695b13ef4cdab556e61d8b2d22d6.png)
...
![Image 1](_media/2c1a174db95f487e9be177d44cdb0773.png)
...
![Image 1](_media/0e53ad1998944c179a5794a249335670.png)
...
> 预告：本小节定位为基础科普向，对 Push Cache 有深入挖掘兴趣的同学，强烈推荐拓展阅读 Chrome 工程师 Jake Archibald 的这篇 [HTTP/2 push is tougher than I thought(opens new window)][http_2 push is tougher than i thought_opens new window]。
...
![Image 1](_media/55dd308a6cb84d1ba73746049f927ed6.png)
...
![Image 1](_media/b5cab4b44c0f4e4d85f5828497d1a0da.png)
...
![Image 1](_media/e6e7be9803154bebbfcaf8ba2b845646.png)
...
> 浏览器缓存/存储技术的出现和发展，为我们的前端应用带来了无限的转机。近年来基于缓存/存储技术的第三方库层出不绝，此外还衍生出了 [PWA(opens new window)][pwa_opens new window] 这样优秀的 Web 应用模型。可以说，现代前端应用，尤其是移动端应用，之所以可以发展到在体验上叫板 Native 的地步，主要就是仰仗缓存/存储立下的汗马功劳。
...
![Image 1](_media/65ffb63a887f4e87944cb90c5dd8954a.png)
...
![Image 1](_media/179e094ada5c468f98af36a46974d9e0.png)

我们点击 preview，可以看到业务服务器确实是返回给了我们一个尚未被静态资源加持过的简单 HTML 页面，所有的图片内容都是先以一个 div 占位：

![Image 1](_media/afe5eb820aad4533ba0356321927f967.png)

相应地，我们随便点开一个静态资源，可以看到它都是从 CDN 服务器上请求来的。

比如说图片：

![Image 1](_media/ed48babef27547e982ff77b327b97bb8.png)

再比如 JS、CSS 文件：

![Image 1](_media/ab950109867f428d817392152e87b49e.png)

![Image 1](_media/1a0a8e9798804fe98172346deed22d98.png)
...
![Image 1](_media/7e63bcd34b2b476487e4236becf9eb5a.png)
...
在服务端的入口文件中，我引入这个组件，对它进行渲染：
    ...
    const Page = ` <html> <head> <title>test</title> </head> <body> <span>服务端渲染出了真实DOM: </span> ${RDom} </body> </html> `
...
根据我们的路由配置，当我访问 [http://localhost:8000/index(opens new window)][http_localhost_8000_index_opens new window] 时，就可以呈现出服务端渲染的结果了：

![Image 1](_media/0caa8a0d8d2f4d808178bd5679959636.png)
...
> 其实是一个套路，这里基于 [Vue SSR 指南(opens new window)][vue ssr _opens new window] 中官方给出的例子为大家讲解 Vue 中的实现思路（思路见注释）。
...
![Image 1](_media/8259dca69be946dda92af82dd004f3ba.png)
...
![Image 1](_media/474905a554354be091ebde90f1ecd089.png)
...
![Image 1](_media/458248282f664e8d80ea01e1dc110bf2.png)
...
![Image 1](_media/6c32618ecbc043afa1250ef04b282a00.png)
...
我们通过一个 🌰 来理解一下这个机制：
    ...
    <head>
      ...
      <title>JS阻塞测试</title>
      <style> #container { background-color: yellow; width: 100px; height: 100px; } </style>
      <script> // 尝试获取container元素 var container = document.getElementById("container") console.log('container', container) </script>
    </head>
    <body>
      <div id="container"></div>
      <script> // 尝试获取container元素 var container = document.getElementById("container") console.log('container', container) // 输出container元素此刻的背景色 console.log('container bgColor', getComputedStyle(container).backgroundColor) </script>
      <style> #container { background-color: blue; } </style>
    </body>
    </html>

三个 console 的结果分别为：

![Image 1](_media/d66556a0ef4e4b5494e48643bd42697a.png)
...
![Image 1](_media/c17c876c3454445a9662a578da25e590.png)
...
![Image 1](_media/ab27d1cb420b4eeca84ab70b2a9c95b9.png)
...
这个思路，在 [DOM Fragment(opens new window)][dom fragment_opens new window] 中体现得淋漓尽致。
...
![Image 1](_media/15ff482dcc424d70ada6d93b9d97798c.png)
...
![Image 1](_media/c331bdda20e9444ba0b4c48b7b47c42e.png)
...
- 本小节我们从性能优化的角度出发，通过解析 Vue 源码，对异步更新这一高效的 `DOM` 优化手段有了感性的认知。同时帮助大家进一步熟悉了 `micro` 与 `macro` 在生产中的应用，加深了对 `Event Loop` 的理解。事实上，Vue 源码中还有许多值得称道的生产实践，其设计模式与编码细节都值得我们去细细品味。对这个话题感兴趣的同学，课后不妨移步 [Vue 运行机制解析(opens new window)][vue_opens new window] 进行探索。
...
有时我们想要通过多次计算得到一个元素的布局位置，我们可能会这样做：
    ...
    <head>
      ...
      <style> #el { width: 100px; height: 100px; background-color: yellow; position: absolute; } </style>
    </head>
    <body>
      <div id="el"></div>
      <script> // 获取el元素 const el = document.getElementById('el') // 这里循环判定比较简单，实际中或许会拓展出比较复杂的判定需求 for(let i=0;i<10;i++) { el.style.top = el.offsetTop + 10 + "px"; el.style.left = el.offsetLeft + 10 + "px"; } </script>
    </body>
    </html>
...
优化成一个有 `class` 加持的样子：
    ...
    <head>
      ...
      <meta http-equiv="X-UA-Compatible" content="ie=edge">
      <title>Document</title>
      <style> .basic_style { width: 100px; height: 200px; border: 10px solid red; color: red; } </style>
    </head>
    <body>
      <div id="container"></div>
      <script> const container = document.getElementById('container') container.classList.add('basic_style') </script>
    </body>
    </html>
...
![Image 1](_media/da135196ce69438dbfe123d8d3be6059.png)
...
![Image 1](_media/f947f03993664c058484c57a152b974f.png)

大家留意一栏文章右侧可能会出现的图片，这里咱们给个特写：

![Image 1](_media/746c1fca8d334be389dece31965a7435.png)
...
![Image 1](_media/36f4781ab61945d6972e63735e3449e6.png)

我们把代码提出来看一下：

    <div data-v-b2db8566="" data-v-009ea7bb="" data-v-6b46a625="" data-src="https://user-gold-cdn.xitu.io/2018/9/27/16619f449ee24252?imageView2/1/w/120/h/120/q/85/format/webp/interlace/1" class="lazy thumb thumb" style="background-image: none; background-size: cover;">
    </div>

- 我们注意到 `style` 内联样式中，背景图片设置为了 `none`。也就是说这个 `div` 是没有内容的，它只起到一个**占位**的作用。
- 这个“占位”的概念，在这个例子里或许体现得不够直观。最直观的应该是淘宝首页的 HTML Preview 效果：

![Image 1](_media/1b380bbc02554e54a5d72d98e20e6aaf.png)

- 我们看到，这个还没来得及被图片填充完全的网页，是用大大小小的空 `div` 元素来占位的。掘金首页也是如此。
- 一旦我们通过滚动使得这个 `div` 出现在了可见范围内，那么 `div` 元素的内容就会发生变化，呈现如下的内容：

![Image 1](_media/d9c64420088e4a4083cbede6961840ae.png)
...
![Image 1](_media/216afa38ebd14596a6e3fd554fec368d.png)
...
- 我们在 `index.html` 中，为这些图片预置 `img` 标签：
  ...
  <head>
    ...
    <style> .img { width: 200px; height:200px; background-color: gray; } .pic { // 必要的img样式 } </style>
  </head>
  <body>
    <div class="container">
      <div class="img">
...
![Image 1](_media/e9ac9ae1550b485283be79b2091080a8.png)
...
![Image 1](_media/1f9b5c2998394d50ae2344e810ccf3fb.png)

![Image 1](_media/135f1d9c6766472ab82b85b7367eca22.png)

**2. 简要分析**

这里我打开掘金首页，选中 `Performance` 面板中的圆箭头，来看一下页面加载过程中的性能表现：

![Image 1](_media/4dfa7249f39a45b3a571f43725b301c5.png)

从上到下，依次为概述面板、详情面板。下我们先来观察一下概述面板，了解页面的基本表现：

![Image 1](_media/a7f276e780024d71882d262ee5b4319b.png)
...
![Image 1](_media/644b8a0e26e546f28670442d3e975f44.png)
...
![Image 1](_media/c9562837f87c40c8a6446b73bdaf354a.png)
...
![Image 1](_media/6310eb9928804c4baf822ae19fccaa06.png)
...
![Image 1](_media/6b3b4ab1dc8c4aac8bb30efad6c3b6e3.png)

**2. 关键时间节点**

在 `performance` 的 `timing` 属性中，我们可以查看到如下的时间戳：

![Image 1](_media/d40725c78b3847e68cce185e05dccc24.png)

这些时间戳与页面整个加载流程中的关键时间节点有着一一对应的关系：

![Image 1](_media/5e14a4894ba84968bfe18c44841a024c.png)
...
- [Performance 官方文档(opens new window)][performance _opens new window]
- [使用 Lighthouse 审查网络应用(opens new window)][lighthouse _opens new window]
- [MDN Performance API 介绍(opens new window)][mdn performance api _opens new window]

[1.png]: https://s.poetries.work/gitee/2020/07/performance/1.png
[2.png]: https://s.poetries.work/gitee/2020/07/performance/2.png
[webpack _opens new window]: https://juejin.im/book/5a6abad5518825733c144469/section/5a6abad5518825732e2f8546#heading-2
[webpack-bundle-analyzer_opens new window]: https://www.npmjs.com/package/webpack-bundle-analyzer
[3.png]: https://s.poetries.work/gitee/2020/07/performance/3.png
[opens new window]: http://www.stevesouders.com/blog/2013/04/26/i/
[http-archive_opens new window]: https://httparchive.org/reports/page-weight#bytesTotal
[109.png]: https://s.poetries.work/gitee/2020/09/109.png
[110.png]: https://s.poetries.work/gitee/2020/09/110.png
[4.png]: https://s.poetries.work/gitee/2020/07/performance/4.png
[5.png]: https://s.poetries.work/gitee/2020/07/performance/5.png
[6.png]: https://s.poetries.work/gitee/2020/07/performance/6.png
[7.png]: https://s.poetries.work/gitee/2020/07/performance/7.png
[8.png]: https://s.poetries.work/gitee/2020/07/performance/8.png
[opens new window 1]: http://www.iconfont.cn/
[9.png]: https://s.poetries.work/gitee/2020/07/performance/9.png
[10.png]: https://s.poetries.work/gitee/2020/07/performance/10.png
[url-loader_opens new window]: https://github.com/webpack-contrib/url-loader
[11.png]: https://s.poetries.work/gitee/2020/07/performance/11.png
[12.png]: https://s.poetries.work/gitee/2020/07/performance/12.png
[13.png]: https://s.poetries.work/gitee/2020/07/performance/13.png
[14.png]: https://s.poetries.work/gitee/2020/07/performance/14.png
[15.png]: https://s.poetries.work/gitee/2020/07/performance/15.png
[16.png]: https://s.poetries.work/gitee/2020/07/performance/16.png
[17.png]: https://s.poetries.work/gitee/2020/07/performance/17.png
[18.png]: https://s.poetries.work/gitee/2020/07/performance/18.png
[19.png]: https://s.poetries.work/gitee/2020/07/performance/19.png
[20.png]: https://s.poetries.work/gitee/2020/07/performance/20.png
[http_2 push is tougher than i thought_opens new window]: https://jakearchibald.com/2017/h2-push-tougher-than-i-thought/
[21.png]: https://s.poetries.work/gitee/2020/07/performance/21.png
[22.png]: https://s.poetries.work/gitee/2020/07/performance/22.png
