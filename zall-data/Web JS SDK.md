...
数据分析工作台 SDK 初始化成功后，即可以通过 zall.track(event_name[, properties][, callback])记录事件（触发的事件会存储到数据分析工作台系统的 events 表中）：
...
**前端方式：**
增加了获取匿名ID的方法 zall.quick('getAnonymousID'); 返回匿名 id （SDK 版本 1.13.4 及以上支持），调用这个方法时，可能 SDK 还未初始化成功，建议将此方法放在下面代码中。
