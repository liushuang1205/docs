...
<center><font color=gray size=3><b>启动产品→搜索/收藏/推荐→点击搜索结果/点击收藏的内容/点击推荐内容→开始阅读</b></font></center>
...
![](.\配图\Session分析\查询条件配置.gif)

<center><font color=gray size=2><b>查询条件配置</b></font></center>

用户进入**事件分析**模块，在**分析对象**处选择**Session**，即进入session分析模块。在使用session分析之前，用户需要结合产品及业务特点明确需要分析的问题，并根据目标问题创建Session，Session的创建方法请参考[Session管理](Session管理.md)。
